...
export function createNightSky(ctx: SceneContext) {
  ...
  const fireMat = new T.ShaderMaterial({
    ...
    vertexShader: `uniform float uTime;uniform float uNight;varying float vAlpha;void main(){vec3 p=vec3(-3.3+position.x*6.,1.3+position.y*2.9,position.z*4.5-2.);p+=vec3(sin(uTime*.4+position.z*25.)*.27,sin(uTime*.7+position.x*28.)*.18,cos(uTime*.36+position.y*20.)*.24);vec4 mv=modelViewMatrix*vec4(p,1.);gl_Position=projectionMatrix*mv;gl_PointSize=clamp(72./-mv.z,2.,7.);vAlpha=uNight*(.35+.65*pow(.5+.5*sin(uTime*1.5+position.x*40.),2.));}`,
