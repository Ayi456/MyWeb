...
- **云上电台**：标题下方展开电台，点击播放。支持上一首、下一首、进度拖动和音量调节。播放模式按钮在顺序、循环、随机之间切换，默认循环；顺序模式播到歌单末尾停止，其余模式曲终继续下一首。歌曲无法播放（如仅限 VIP）时会提示并在 1.5 秒后自动跳到下一首。收起电台、隐藏界面或打开寄信窗不打断音乐。页面刷新后保持静音，音乐不跟随场景时间倍率变化。
