...
**Postcards from the Sky** — 一张可以走进去的春日明信片。React 负责界面，TypeScript + Three.js r186 负责可交互的童话微缩世界，Vite 构建前端，Vercel Function 为「云上电台」提供公开歌单和播放地址。

本项目从根目录的 `spring-post-office.html` 迁移，当前场景以云鲸为造型基准，使用连续浮岛地形、圆润树冠与云团、哑光材质、柔化建筑和家具；主场景仍使用固定种子 **314159** 与自定义 Shader。在此基础上扩展了花园副岛、灯塔小站、群岛邮路、彩虹和云上电台，并加入邮局历书、本机邮戳收藏、明信片导出与场景分享链接。原 HTML 原样保留，不参与生产构建。前端代码和字体不依赖外部 CDN；音乐由网易云音频 CDN 提供。
...
- **岛上散步**：在指南选择「开始岛上散步」，从小兔的高度用 WASD、方向键或屏幕方向按钮走动，拖动看四周。脚步贴合主岛连续地面，建筑、树干、长椅、围栏、水池和岛边限制移动。打开面板、失焦或后台时停止输入；「结束散步」回到原环视，复位与登船也会退出。移动速度不受时间倍率影响，暂停世界后仍可主动走动，无自动镜头摇晃；「复制此刻」可分享位置与朝向，非法落点回到安全起点。
...
| `src/scene/utils`               | LCG 随机数、共享方块、圆角与椭球 InstancedMesh 批渲染 |
...
- 高：DPR 上限 1.6，1024 阴影，1250 花瓣；中：1.2 / 768 / 850；低：0.9 / 512 / 480。相机、主模型和场景比例不会随质量改变。
...
- 竖屏采用相机随动天空与雾密度补偿，修复原型在拉远时的天空裁切和过度雾化；默认角度稍向正面调整，避免灯塔被右侧按钮遮住。桌面主岛默认相机保持不变，茶山视角从侧面避开驿站。
