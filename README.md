...
这是虚拟邮局，**信件文字不会发送到外部服务，也不会保存在浏览器中**。不调用邮件或 AI API，不收集姓名、邮箱、联系方式。信件输入只在当前页面内存中存在；提交后清空输入，不写入 localStorage、URL 或控制台，不用 innerHTML 渲染用户文字。邮戳 ID、固定回信文案的索引和累计寄信次数保存在此设备的 localStorage，可在信箱中清空。页面刷新后本次寄信次数清零，累计次数保留。所有模型和纹理由程序生成，图标在仓库内。
