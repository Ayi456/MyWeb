...
这是虚拟邮局，**信件文字不会自动发送到外部服务，也不会存入浏览器持久存储**。不调用邮件或 AI API，不收集姓名、邮箱、联系方式。信件文字只在当前页面内存中存在；提交后清空输入，不写入 localStorage、URL 或控制台，不用 innerHTML 渲染用户文字。用户拍照时可以逐次勾选把最近一封信的文字印进本地导出的 PNG；用户随后主动分享图片时，图片内容由用户决定。邮戳 ID、固定回信文案的索引和累计寄信次数保存在此设备的 localStorage，可在信箱中清空。界面偏好（含跟随现实开关）也只存在本机，不存地理位置。页面刷新后本次寄信次数清零，累计次数保留。所有模型和纹理由程序生成，图标在仓库内。
