...
初稿整理于 2026-09-22，基于提交 `d21ceca` 的代码现状。实施进度随每次提交登记在第 7 节。

**当前进度**：第 5 节的第 1–4 阶段已全部完成；第 5 阶段的新事件、冬夜极光、情境文案、视角预设与巡游、群岛寻宝、留声机与灯塔低鸣已完成。2026-09-26 用户指定的四项（2 驿站岛、3 岛上散步、5 音乐律动、6 Three.js 升级）均已完成，逐项更新文档并提交推送。多频道、歌词与双语仍待实施。第 7 节记录每一项的状态与验证。第 0–6 节保留初稿的需求分析，里面的「现在」「没有」和源码行号均指 2026-09-22 的历史基线，不能当作当前缺陷清单。
...
- 初稿只提思路；后续实现和验证进度记录在第 7 节与 [验证记录](VALIDATION.md)。

## 0. 结论摘要

以下是 2026-09-22 的初稿判断；其中回信反馈、限流退避、键盘热点、本机留存、分享和 CI 等已在第 7 节标为完成。网站当时的骨架已经完整：体素世界、四季昼夜、事件、热点、寄信回信、邮戳、电台、画质自适应都在。下一步规划是四件事：
...
| 月相与中秋满月 | 已完成 | 以 29.53 日平均朔望周期在现有月亮材质中绘制盈亏，零新增 draw call；中秋强制满月，经典模式保留原有月面 |
| 拍照：导出此刻明信片 | 已完成 | 侧栏相机导出当前画面，2D 合成季节邮框、日期、邮戳；支持系统文件分享或 PNG 下载；信件文字每次默认关闭、单独勾选 |
| 复制此刻 | 已完成 | 一键复制季节、时辰、预设或自定义环视角度与活动事件；仅白名单参数可加载，URL 不带信件文字 |
| PWA 与离线 | 已完成 | manifest 与四枚图标；构建期生成带内容版本的 `sw.js`，预缓存页面外壳与全部产物，导航网络优先、4 秒回退缓存；`/api/`、`/_vercel/` 不拦截；电台离线时显示「离线中」、不消耗跳过次数，联网后自动重连 |
| 三个新事件：孔明灯、晨雾云海、夏夜雷阵雨 | 已完成 | 按时辰与季节进入事件池；复用节日天灯、云层与雨粒子，增加雷声和字幕；减少动态效果下柔和变亮，暂停冻结，分享链接可重演；生命周期与浏览器验证见验证记录 |
| 冬夜极光 | 已完成 | 冬季晴夜偶遇，单网格加色混合与程序噪声生成绿紫光带；共享模拟时钟，减少动态效果时静止，雨天和换季隐去；不消耗主岛随机序列，分享参数支持 `aurora` |
| 情境文案系统 | 已完成 | 11 个热点按季节、昼夜、天气、风、事件、寄信与收藏状态分别轮换；航程字幕和写信邀请随情境变化；追加云鲸、雨、夜与晨间固定回信，保留旧索引；手动及自然换季触发轻抖和应季粒子，减少动态效果时减弱 |
| 视角预设扩展、双击靠近与闲置巡游 | 已完成 | 花园、灯塔、茶山、温泉村视角在指南中键盘可达且可分享；双击热点按目标平滑靠近，双击飞艇登船；闲置 8.5 秒后就近接入 150 秒闭合样条巡游，操作、面板、暂停与减少动态效果停止；主岛构图与模型未改 |
| 群岛寻宝 | 已完成 | 指南主动开始四步线索，串起花园风车、灯塔、茶山与温泉村；点击和键盘共用热点，每周一本地日期换路线，24 周顺序不重复；版本化本机进度、刷新续玩、寻宝邮戳与清空收藏联动；只复用现有地标，不改模型和随机序列 |
| 花园岛留声机与灯塔雾号 | 已完成 | 花园新增两组共享资源的体素批次，画布点击与指南键盘按钮控制现有电台播放/暂停；加载取消、失败重试和自动播放拒绝沿用播放器规则；唱片随实际播放转动，暂停与减少动态效果冻结；夜雾、雨雾或冬雾下按模拟时钟合成灯塔低鸣，可静音显示字幕 |
| 驿站岛与索道缆车 | 已完成 | 追加无随机调用的体素驿站、双索道与邮包缆车，44 秒闭合往返、两端停靠与缓动；暂停、后台和减少动态效果冻结；新增可分享预设、热点、键盘按钮与巡游停靠点 |
| 第一人称岛上散步 | 已完成 | 小兔高度镜头，WASD/方向键与 44px 触屏方向按钮；复用实际地面高度和静态体素碰撞，岛边与水池防跌落，面板/失焦/后台清除输入；结束、复位和登船返回环视，白名单链接分享落点与朝向 |
| 音乐律动 | 已完成 | 电台主动开启、默认关闭并本机留存；逐曲 CORS 检查后用真实低频能量轻微驱动灯笼与螺旋桨；跨域拒绝或 WebAudio 不可用回退普通播放，播放中开启保留进度；暂停冻结、减少动态效果关闭 |
| Three.js r160 → r186 | 已完成 | 运行库与类型固定为 0.186.0，更新 PCF 阴影与 WebGL 2 失败提示；保持 HDR/ACES/sRGB 管线、模型和随机序列，原四张基线与 45 项浏览器回归通过；三次生命周期、12 封信资源回收、context loss 恢复与开发/生产真实版本验证，保存升级对照截图 |

第 3.5 节的小玩法本轮只完成群岛寻宝；春日回声与拾花瓣仍待实施。

2026-09-24 验证：TypeScript、ESLint、Prettier、78 项单元测试、生产构建和 15 项 Playwright 测试通过；四张主岛视觉基线未变。开场、春风、声音邀请与字幕作为前一轮工作一次补交；其后每项单独提交。2026-09-25 起的条目各自在 [验证记录](VALIDATION.md) 中登记检查结果，最新总数见该文件开头。尚未核验移动设备真机。
