...
将左右分开的嘴线改为一条贴合脸部曲面的连续微笑，嘴角略微上扬。增加正面遮挡检查，确保整条嘴线位于身体表面之外；浏览器中检查了正面、斜侧面和侧面。

以下是使用实际云鲸模型的独立近景检查。

![正面连续微笑](./whale-front.png)

![斜侧面](./whale-quarter.png)

## 全屏冬雪

冬季飘雪独立于主岛的花瓣范围，覆盖当前镜头的整个视野。近处雪花稍大，远处更细小，边缘柔和，并保留建筑与岛屿的遮挡。转动、缩放和切换远岛视角后仍有全屏飘雪，横屏与竖屏都适用。

雪花跟随模拟暂停；简化动态设置降低飘落速度，低画质减少数量。春秋花瓣与树叶保持原来的主岛范围，季节过渡时与雪花淡入淡出。

![冬季全景](../screenshots/three-r186-winter.png)

[灯塔岛飘雪](./snow-lighthouse.png) · [390 像素竖屏](./snow-mobile.png)

验证了桌面全景、灯塔、茶山、手机竖屏和手动转动镜头。浏览器检查增加了冬季粒子切换、低画质粒子数量、暂停雪花运动及恢复春季花瓣，并通过完整场景生命周期检查，详见 [本次报告](./snow-browser-checks.txt)。141 项单元测试、构建与 lint 通过；冬季截图基准已更新并人工检查，本次未运行完整 Playwright 套件。
