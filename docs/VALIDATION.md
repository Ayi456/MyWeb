...
- 已运行 `npm run music:probe`：公开歌单歌曲 3342319503 的 `m801.music.126.net` HTTPS HEAD 返回 200、`Access-Control-Allow-Origin: *`、`audio/mpeg`。脚本只打印歌曲 ID、主机和响应头，不打印完整播放 URL 或用户数据；这次探测不代表所有歌曲或未来响应。客户端逐曲以无凭据 CORS HEAD 核验，最多等待 3 秒，取消加载会中止检查。
...
- 直接复用主岛生成时的草地体素坐标与顶面高度，不改变地形、随机调用或绘制批次。静态碰撞从邮局、树干、长椅、秋千、邮筒和围栏的实际实例矩阵提取；水池与泄水区域额外禁止行走。小兔半径 0.13、眼高 0.72；周边八点检测保证身体不会越过体素岛边，按轴移动可沿墙滑行。
...
- 花园小路旁新增木盒、黄铜喇叭与唱片；两组体素批次共用现有几何体和材质，不新增贴图、外部文件或主岛随机调用，最多增加两次可见绘制，不投射额外阴影。场景资源跟踪器统一释放共享资源，重复释放单元测试通过。新增热点追加在键盘按钮末尾，原有数字键映射保持稳定，双击映射到花园视角。
...
- `script.ts` 以纯函数按季节、昼夜、雨、风、活动事件、寄信数、回信和邮戳状态选择文案；11 个热点分别轮换，点击另一热点不会跳过当前热点的下一句。写信邀请和航程字幕也随情境变化，航线采样函数与 76 秒航程不变。
- 回信新增云鲸、雨、夜色和清晨的固定句子，每季原有索引 0–4 完全保留；所有追加文案能编码、恢复，信件文字不进入文案选择或持久存储。浏览器实测寄信、收取带云鲸的回信、刷新后重新读到。
- 手动及自然跨季都会触发树的轻抖、应季粒子和沙沙音效；暂停冻结，减少动态效果下冲量减弱。视觉验证页通过初始季节构建静止画面，避免把用户主动换季的反应混入参考基线。
- TypeScript、ESLint、Prettier、Vitest **100 / 100**、生产构建和 Playwright **24 / 24** 通过，四张主岛基线未变。首轮信箱刷新测试用了错误的弹窗名称；修正测试定位后完整回归通过。
...
2026-09-24：浏览器导出当前 WebGL 画面，检查 PNG 文件头、实际文件体积与图像像素；信件文字默认不进入 2D 图片绘制，第二次打开仍默认关闭，逐次勾选后才绘入，且 localStorage 收藏不含文字。相机与指南按钮在桌面、手机竖屏与横屏视口都可点击。系统文件分享是否弹出需在支持文件分享的真机上另验。

## 迭代验证：复制此刻

2026-09-24：单元测试验证链接只接受有限时刻、四季索引、视角预设、活动事件与有界自定义镜头数值，舍弃旧查询参数和信件文字。浏览器实测复制链接后重现冬季星夜、近景视角与云鲸事件；手动环视角度能复现，寄出的信件文字未进入链接。

## 迭代验证：PWA 与离线

2026-09-25：生产构建输出 `sw.js`、`manifest.webmanifest` 与四枚图标，预缓存 11 个文件（页面外壳、JS/CSS、图标、manifest），不含 `og.jpg` 与 source map。Playwright 在 4174 端口起独立生产预览，等 service worker 接管后切断网络并刷新：场景照常渲染，可以寄信，电台显示「离线中」；恢复网络后电台自动重连。本地预览发现服务器对模块脚本返回 `Vary: Origin`，带 `crossorigin` 的请求因此匹配不到预缓存，已改为忽略 Vary 匹配（资源均同源且文件名带内容哈希）。单元测试覆盖预缓存清单与 `/api/`、`/_vercel/` 放行。Vercel 线上的安装提示和 iOS「添加到主屏幕」仍需在真机上确认。

同日修复一处与日期相关的测试：中秋当天寄信还会获得限定邮戳，收藏测试改为按名称检查樱花邮戳。
